...
The **Docker Desktop** interface displays the list of local images, including `iris-ml-app:latest` created for this assignment (size: 970.23 MB). Its presence alongside other images such as `postgres` and `apache/airflow` confirms that the build was successfully stored locally and is available to be run or pushed to a container registry if desired.
